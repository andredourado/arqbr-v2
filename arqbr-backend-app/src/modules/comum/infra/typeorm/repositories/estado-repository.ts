    ...
    try {
      ...
      const estados = await query
        .andWhere(new Brackets(query => {
          query.andWhere('CAST(est.nomeEstado AS VARCHAR) ilike :search', { search: `%${search}%` })
          query.orWhere('CAST(est.uf AS VARCHAR) ilike :search', { search: `%${search}%` })
        }))
        .addOrderBy('est.nomeEstado', columnOrder[0])
