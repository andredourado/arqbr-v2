...
def pages_split(pdf_file, target_dir, spaces, bucket_name, prefix):
    ...
    for current_page in range(1, max_pages + 1):
        ...
        val128 = pyzbar.decode(pages[0], symbols=[ZBarSymbol.CODE128])

        if (val != []) and ((b'ARQBR_' in val[0][0]) or (b'SEPARADOR' in val[0][0]) or (len(val128[0][0]) == 47)):
            ...
            if (b'SEPARADOR' in val[0][0]) or (len(val128[0][0]) == 47):
...
while True:
    ...
    for file in files:
        ...
        if (os.path.basename(file.key) != ''):
            ...
            pages_split("./ARQBRCX4630-000000617.pdf", "/tmp/pdfs-splitted", spaces, 'vamilly-arqbr', 'arquivos-pdf-separados/')
