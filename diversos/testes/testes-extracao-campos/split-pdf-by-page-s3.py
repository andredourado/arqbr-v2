...
try:
    ...
    if e.response['Error']['Code'] == "404":
        ...
        print("Erro ao acessar o bucket:", e)
        raise
...
directory = '/tmp/pdfs-separados-from-s3'

if not os.path.exists(directory):
    os.makedirs(directory)

directory = '/tmp/pdfs-splitted-by-page'
...
while True:
    ...
    not_sorted_files = bucket.objects.filter(Prefix=source_directory_name)
    files = sorted(not_sorted_files, key=lambda obj: obj.key)

    files_list = ''
    for file in files:
        if file.key not in already_splitted_files:
            print('----')
            print(file.key)
    ...
    already_splitted_files += files_list

    bucket.put_object(Key=already_splitted_files_name, Body=already_splitted_files)
