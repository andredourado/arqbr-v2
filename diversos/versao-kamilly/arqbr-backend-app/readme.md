...
O projeto **arqbr**, aplicação de gerenciamento de documentso, consiste em uma API REST desenvolvida com o framework Node.js/Express em TypeScript, banco de dados PostegreSQL, acessado através do ORM TypeORM e upload de arquivos através da biblioteca Multer.
...
```
  ...
  postgres=# create user arqbr_user with encrypted password '123456'
  postgres=# create database arqbr_db
  postgres=# grant all privileges on database arqbr_db to arqbr_user
  postgres=# exit
```
...
```
  ...
  [+] Running 3/3
   - Network network_arqbr     Created                                  0.0s
   - Container database_arqbr  Created                                  0.0s
   - Container redis_arqbr     Created                                  0.0s
  Attaching to database_arqbr, redis_arqbr
  database_arqbr  |
  database_arqbr  | PostgreSQL Database directory appears to contain a database ˜˜˜
  database_arqbr  |
  ˜˜˜
```
...
```
  ...
  bec89ece9546   postgres       ˜˜˜   Up 5 seconds   0.0.0.0:5432->5432/tcp   database_arqbr
  1f1efb10abbd   redis:alpine   ˜˜˜   Up 5 seconds   0.0.0.0:6379->6379/tcp   redis_arqbr
```

Para verificar o log de um container, em linha de comando digite o comando utilizando o nome do container desejado:

```
  docker logs database_arqbr
...
```
  docker exec -it database_arqbr /bin/bash
...
```
  ...
  [+] Running 3/3
   - Container database_arqbr  Removed                                  3.1s
   - Container redis_arqbr     Removed                                  3.2s
   - Network network_arqbr     Removed                                  0.2s
```
...
```
  ...
  [+] Running 3/3
   - Network network_arqbr     Started                                  0.0s
   - Container database_arqbr  Started                                  0.7s
   - Container redis_arqbr     Started                                  0.9s
```
