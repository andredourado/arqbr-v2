...
class PlantaRepository implements IPlantaRepository {
  ...
  async list (
    ...
    try {
      let plantas = await this.repository.createQueryBuilder('pla')
        .select([
          'pla.id as "id"',
          'a.id as "unidadeId"',
        ...
        .orWhere('pla.nome ilike :search', { search: `%${search}%` })
      ...
      if (plantas.length > rowsPerPage) {
        plantas = plantas.slice(offset, offset + rowsPerPage)
      }
