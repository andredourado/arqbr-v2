...
class SolicitanteRepository implements ISolicitanteRepository {
  ...
  async list (
    ...
    try {
      let solicitantes = await this.repository.createQueryBuilder('sol')
        .select([
          'sol.id as "id"',
          'a.id as "clienteId"',
        ...
        .orWhere('sol.nome ilike :search', { search: `%${search}%` })
        .orWhere('sol.email ilike :search', { search: `%${search}%` })
      ...
      if (solicitantes.length > rowsPerPage) {
        solicitantes = solicitantes.slice(offset, offset + rowsPerPage)
      }
