...
class VeiculoRepository implements IVeiculoRepository {
  ...
  async list (
    ...
    try {
      let veiculos = await this.repository.createQueryBuilder('vei')
        .select([
          'vei.id as "id"',
          'vei.descricao as "descricao"',
          'vei.capacidade as "capacidade"',
        ])
        .where('vei.descricao ilike :search', { search: `%${search}%` })
        .addOrderBy('vei.descricao', columnOrder[0])
        .addOrderBy('vei.capacidade', columnOrder[1])
        .take(rowsPerPage)
        .skip(offset)
        .getRawMany()
      ...
      if (veiculos.length > rowsPerPage) {
        veiculos = veiculos.slice(offset, offset + rowsPerPage)
      }
