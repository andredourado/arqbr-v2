...
class VolumeRepository implements IVolumeRepository {
  ...
  async list (
    ...
    try {
      let volumes = await this.repository.createQueryBuilder('vol')
        .select([
          'vol.id as "id"',
          'a.id as "coletaId"',
      ...
      if (volumes.length > rowsPerPage) {
        volumes = volumes.slice(offset, offset + rowsPerPage)
      }
