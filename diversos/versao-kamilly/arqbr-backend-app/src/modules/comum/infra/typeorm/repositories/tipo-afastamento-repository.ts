...
class TipoAfastamentoRepository implements ITipoAfastamentoRepository {
  ...
  async list (
    ...
    try {
      let tiposAfastamento = await this.repository.createQueryBuilder('tip')
        .select([
          'tip.id as "id"',
          'tip.descricao as "descricao"',
        ])
        .where('tip.descricao ilike :search', { search: `%${search}%` })
      ...
      if (tiposAfastamento.length > rowsPerPage) {
        tiposAfastamento = tiposAfastamento.slice(offset, offset + rowsPerPage)
      }
