...
class CampoDocumentoRepositoryInMemory implements ICampoDocumentoRepository {
  ...
  async list (
    ...
    filteredCamposDocumento = filteredCamposDocumento.filter((campoDocumento) => {
      ...
      if (campoDocumento.identificador.includes(search)) return true
  ...
  async select (filter: string): Promise<HttpResponse> {
    ...
    filteredCamposDocumento = filteredCamposDocumento.filter((campoDocumento) => {
      ...
      if (campoDocumento.identificador.includes(filter)) return true
  ...
  async update ({
    ...
    this.camposDocumento[index].versaoDocumentoId = versaoDocumentoId
    this.camposDocumento[index].nomeCampo = nomeCampo
    this.camposDocumento[index].identificador = identificador
    this.camposDocumento[index].cantoSuperiorX = cantoSuperiorX
    this.camposDocumento[index].cantoSuperiorY = cantoSuperiorY
    this.camposDocumento[index].cantoInferiorX = cantoInferiorX
    this.camposDocumento[index].cantoInferiorY = cantoInferiorY
    this.camposDocumento[index].conteudoParaValidacao = conteudoParaValidacao
    this.camposDocumento[index].pessoaId = pessoaId
