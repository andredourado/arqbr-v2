...
class JornadaRepository implements IJornadaRepository {
  ...
  async list (
    ...
    try {
      let jornadas = await this.repository.createQueryBuilder('jor')
        .select([
          'jor.id as "id"',
          'jor.descricao as "descricao"',
        ])
        .where('jor.descricao ilike :search', { search: `%${search}%` })
      ...
      if (jornadas.length > rowsPerPage) {
        jornadas = jornadas.slice(offset, offset + rowsPerPage)
      }
