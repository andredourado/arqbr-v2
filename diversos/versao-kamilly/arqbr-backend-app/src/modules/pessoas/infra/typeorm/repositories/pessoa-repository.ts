...
class PessoaRepository implements IPessoaRepository {
  ...
  async list (
    ...
    try {
      let pessoas = await this.repository.createQueryBuilder('pes')
        .select([
          'pes.id as "id"',
          'a.id as "unidadeId"',
        ...
        .orWhere('pes.nome ilike :search', { search: `%${search}%` })
        .orWhere('pes.email ilike :search', { search: `%${search}%` })
      ...
      if (pessoas.length > rowsPerPage) {
        pessoas = pessoas.slice(offset, offset + rowsPerPage)
      }
