...
export class PlantaListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'quantidadePosicoes', label: 'Quantidade de Posições', type: 'number' }
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getPlanta())
  }
  ...
  getPlanta() {
    this.httpClient.get(`${environment.baseUrl}/plantas?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/plantas?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: PlantaInterface) {
    if (item) this.router.navigate([`plantas/edit/${item.id}`])
    else this.router.navigate([`plantas/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: PlantaInterface) {
    if (item.id) this.router.navigate([`plantas/view/${item.id}`])
    else this.router.navigate([`plantas/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: PlantaInterface) {
  ...
  removeItem(item: PlantaInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/plantas/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: PlantaInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/plantas', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/plantas?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
