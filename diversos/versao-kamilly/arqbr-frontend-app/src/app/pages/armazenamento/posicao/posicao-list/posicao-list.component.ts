...
export class PosicaoListComponent implements OnInit, OnDestroy {
  @ViewChild(PoTableComponent, { static: true }) table: PoTableComponent
  public readonly fields: Array<any> = [
    { property: "id", key: true, visible: false },
    { property: 'unidadeNome', label: 'Unidade' },
    { property: 'plantaNome', label: 'Planta' },
    { property: 'rua', label: 'Rua' },
    { property: 'linha', label: 'Linha' },
    { property: 'coluna', label: 'Coluna' },
    { property: 'posicoes', label: 'Quantidade de Posições', type: 'number' }
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getPosicao())
  }
  ...
  getPosicao() {
    this.httpClient.get(`${environment.baseUrl}/posicoes?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/posicoes?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: PosicaoInterface) {
    if (item) this.router.navigate([`posicoes/edit/${item.id}`])
    else this.router.navigate([`posicoes/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: PosicaoInterface) {
    if (item.id) this.router.navigate([`posicoes/view/${item.id}`])
    else this.router.navigate([`posicoes/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: PosicaoInterface) {
  ...
  removeItem(item: PosicaoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/posicoes/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: PosicaoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/posicoes', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/posicoes?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
