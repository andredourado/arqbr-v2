...
export class DocumentoListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getDocumento())
  }
  ...
  getDocumento() {
    this.httpClient.get(`${environment.baseUrl}/documentos?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/documentos?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: DocumentoInterface) {
    if (item) this.router.navigate([`documentos/edit/${item.id}`])
    else this.router.navigate([`documentos/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: DocumentoInterface) {
    if (item.id) this.router.navigate([`documentos/view/${item.id}`])
    else this.router.navigate([`documentos/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: DocumentoInterface) {
  ...
  removeItem(item: DocumentoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/documentos/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: DocumentoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/documentos', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/documentos?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
