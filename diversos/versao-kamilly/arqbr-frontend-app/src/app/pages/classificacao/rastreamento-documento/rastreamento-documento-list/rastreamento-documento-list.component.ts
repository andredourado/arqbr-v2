...
@Component({
  selector: "/rastreamento-documento-list",
  templateUrl: ".//rastreamento-documento-list.component.html",
})
export class RastreamentoDocumentoListComponent implements OnInit, OnDestroy {
  @ViewChild(PoTableComponent, { static: true }) table: PoTableComponent
  public readonly fields: Array<any> = [
    { property: "id", key: true, visible: false },
    { property: 'volumeIdentificador', label: 'Volume' },
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getRastreamentoDocumento())
  }
  ...
  getRastreamentoDocumento() {
    this.httpClient.get(`${environment.baseUrl}/rastreamento-documentos?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/rastreamento-documentos?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: RastreamentoDocumentoInterface) {
    if (item) this.router.navigate([`rastreamento-documentos/edit/${item.id}`])
    else this.router.navigate([`rastreamento-documentos/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: RastreamentoDocumentoInterface) {
    if (item.id) this.router.navigate([`rastreamento-documentos/view/${item.id}`])
    else this.router.navigate([`rastreamento-documentos/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: RastreamentoDocumentoInterface) {
  ...
  removeItem(item: RastreamentoDocumentoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/rastreamento-documentos/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: RastreamentoDocumentoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/rastreamento-documentos', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/rastreamento-documentos?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
