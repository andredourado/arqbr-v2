...
export class ContratoListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getContrato())
  }
  ...
  getContrato() {
    this.httpClient.get(`${environment.baseUrl}/contratos?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/contratos?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: ContratoInterface) {
    if (item) this.router.navigate([`contratos/edit/${item.id}`])
    else this.router.navigate([`contratos/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: ContratoInterface) {
    if (item.id) this.router.navigate([`contratos/view/${item.id}`])
    else this.router.navigate([`contratos/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: ContratoInterface) {
  ...
  removeItem(item: ContratoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/contratos/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: ContratoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/contratos', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/contratos?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
