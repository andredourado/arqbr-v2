...
export class DepartamentoListComponent implements OnInit, OnDestroy {
  ...
  getDepartamento() {
    this.httpClient.get(`${environment.baseUrl}/departamentos?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/departamentos?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: DepartamentoInterface) {
    if (item) this.router.navigate([`departamentos/edit/${item.id}`])
    else this.router.navigate([`departamentos/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: DepartamentoInterface) {
    if (item.id) this.router.navigate([`departamentos/view/${item.id}`])
    else this.router.navigate([`departamentos/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: DepartamentoInterface) {
  ...
  removeItem(item: DepartamentoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/departamentos/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: DepartamentoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/departamentos', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/departamentos?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
