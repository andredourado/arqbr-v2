...
export class FrequenciaColetaListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'frequenciaDescricao', label: 'Frequência' },
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getFrequenciaColeta())
  }
  ...
  getFrequenciaColeta() {
    this.httpClient.get(`${environment.baseUrl}/frequencia-coletas?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/frequencia-coletas?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: FrequenciaColetaInterface) {
    if (item) this.router.navigate([`frequencia-coletas/edit/${item.id}`])
    else this.router.navigate([`frequencia-coletas/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: FrequenciaColetaInterface) {
    if (item.id) this.router.navigate([`frequencia-coletas/view/${item.id}`])
    else this.router.navigate([`frequencia-coletas/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: FrequenciaColetaInterface) {
  ...
  removeItem(item: FrequenciaColetaInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/frequencia-coletas/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: FrequenciaColetaInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/frequencia-coletas', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/frequencia-coletas?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
