...
export class PontoColetaListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getPontoColeta())
  }
  ...
  getPontoColeta() {
    this.httpClient.get(`${environment.baseUrl}/pontos-coleta?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/pontos-coleta?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: PontoColetaInterface) {
    if (item) this.router.navigate([`pontos-coleta/edit/${item.id}`])
    else this.router.navigate([`pontos-coleta/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: PontoColetaInterface) {
    if (item.id) this.router.navigate([`pontos-coleta/view/${item.id}`])
    else this.router.navigate([`pontos-coleta/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: PontoColetaInterface) {
  ...
  removeItem(item: PontoColetaInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/pontos-coleta/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: PontoColetaInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/pontos-coleta', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/pontos-coleta?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
