...
export class ServicoContratadoListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'servicoDescricao', label: 'Serviço' },
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getServicoContratado())
  }
  ...
  getServicoContratado() {
    this.httpClient.get(`${environment.baseUrl}/servicos-contratados?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/servicos-contratados?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: ServicoContratadoInterface) {
    if (item) this.router.navigate([`servicos-contratados/edit/${item.id}`])
    else this.router.navigate([`servicos-contratados/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: ServicoContratadoInterface) {
    if (item.id) this.router.navigate([`servicos-contratados/view/${item.id}`])
    else this.router.navigate([`servicos-contratados/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: ServicoContratadoInterface) {
  ...
  removeItem(item: ServicoContratadoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/servicos-contratados/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: ServicoContratadoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/servicos-contratados', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/servicos-contratados?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
