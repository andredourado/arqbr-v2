...
export class SolicitanteListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getSolicitante())
  }
  ...
  getSolicitante() {
    this.httpClient.get(`${environment.baseUrl}/solicitantes?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/solicitantes?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: SolicitanteInterface) {
    if (item) this.router.navigate([`solicitantes/edit/${item.id}`])
    else this.router.navigate([`solicitantes/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: SolicitanteInterface) {
    if (item.id) this.router.navigate([`solicitantes/view/${item.id}`])
    else this.router.navigate([`solicitantes/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: SolicitanteInterface) {
  ...
  removeItem(item: SolicitanteInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/solicitantes/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: SolicitanteInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/solicitantes', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/solicitantes?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
