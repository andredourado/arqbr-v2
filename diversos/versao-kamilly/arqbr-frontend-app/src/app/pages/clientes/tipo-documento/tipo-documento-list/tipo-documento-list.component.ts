...
export class TipoDocumentoListComponent implements OnInit, OnDestroy {
  ...
  getTipoDocumento() {
    this.httpClient.get(`${environment.baseUrl}/tipos-documento?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/tipos-documento?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: TipoDocumentoInterface) {
    if (item) this.router.navigate([`tipos-documento/edit/${item.id}`])
    else this.router.navigate([`tipos-documento/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: TipoDocumentoInterface) {
    if (item.id) this.router.navigate([`tipos-documento/view/${item.id}`])
    else this.router.navigate([`tipos-documento/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: TipoDocumentoInterface) {
  ...
  removeItem(item: TipoDocumentoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/tipos-documento/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: TipoDocumentoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/tipos-documento', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/tipos-documento?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
