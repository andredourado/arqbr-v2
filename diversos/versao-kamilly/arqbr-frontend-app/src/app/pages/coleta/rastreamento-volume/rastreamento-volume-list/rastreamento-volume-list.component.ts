...
export class RastreamentoVolumeListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getRastreamentoVolume())
  }
  ...
  getRastreamentoVolume() {
    this.httpClient.get(`${environment.baseUrl}/rastreamento-volumes?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/rastreamento-volumes?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: RastreamentoVolumeInterface) {
    if (item) this.router.navigate([`rastreamento-volumes/edit/${item.id}`])
    else this.router.navigate([`rastreamento-volumes/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: RastreamentoVolumeInterface) {
    if (item.id) this.router.navigate([`rastreamento-volumes/view/${item.id}`])
    else this.router.navigate([`rastreamento-volumes/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: RastreamentoVolumeInterface) {
  ...
  removeItem(item: RastreamentoVolumeInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/rastreamento-volumes/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: RastreamentoVolumeInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/rastreamento-volumes', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/rastreamento-volumes?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
