...
export class VeiculoListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getVeiculo())
  }
  ...
  getVeiculo() {
    this.httpClient.get(`${environment.baseUrl}/veiculos?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/veiculos?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: VeiculoInterface) {
    if (item) this.router.navigate([`veiculos/edit/${item.id}`])
    else this.router.navigate([`veiculos/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: VeiculoInterface) {
    if (item.id) this.router.navigate([`veiculos/view/${item.id}`])
    else this.router.navigate([`veiculos/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: VeiculoInterface) {
  ...
  removeItem(item: VeiculoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/veiculos/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: VeiculoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/veiculos', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/veiculos?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
