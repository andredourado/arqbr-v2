...
export class VolumeListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getVolume())
  }
  ...
  getVolume() {
    this.httpClient.get(`${environment.baseUrl}/volumes?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/volumes?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: VolumeInterface) {
    if (item) this.router.navigate([`volumes/edit/${item.id}`])
    else this.router.navigate([`volumes/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: VolumeInterface) {
    if (item.id) this.router.navigate([`volumes/view/${item.id}`])
    else this.router.navigate([`volumes/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: VolumeInterface) {
  ...
  removeItem(item: VolumeInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/volumes/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: VolumeInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/volumes', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/volumes?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
