...
export class CidadeListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'nomeCidade', label: 'Cidade' }
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getCidade())
  }
  ...
  getCidade() {
    this.httpClient.get(`${environment.baseUrl}/cidades?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/cidades?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: CidadeInterface) {
    if (item) this.router.navigate([`cidades/edit/${item.id}`])
    else this.router.navigate([`cidades/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: CidadeInterface) {
    if (item.id) this.router.navigate([`cidades/view/${item.id}`])
    else this.router.navigate([`cidades/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: CidadeInterface) {
  ...
  removeItem(item: CidadeInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/cidades/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: CidadeInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/cidades', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/cidades?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
