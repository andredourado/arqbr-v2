...
@Component({
  selector: "/composicao-lote-list",
  templateUrl: ".//composicao-lote-list.component.html",
})
export class ComposicaoLoteListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getComposicaoLote())
  }
  ...
  getComposicaoLote() {
    this.httpClient.get(`${environment.baseUrl}/composicao-lotes?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/composicao-lotes?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: ComposicaoLoteInterface) {
    if (item) this.router.navigate([`composicao-lotes/edit/${item.id}`])
    else this.router.navigate([`composicao-lotes/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: ComposicaoLoteInterface) {
    if (item.id) this.router.navigate([`composicao-lotes/view/${item.id}`])
    else this.router.navigate([`composicao-lotes/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: ComposicaoLoteInterface) {
  ...
  removeItem(item: ComposicaoLoteInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/composicao-lotes/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: ComposicaoLoteInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/composicao-lotes', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/composicao-lotes?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
