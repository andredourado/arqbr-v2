...
export class EstadoListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'nomeEstado', label: 'Nome do Estado' }
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getEstado())
  }
  ...
  getEstado() {
    this.httpClient.get(`${environment.baseUrl}/estados?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/estados?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: EstadoInterface) {
    if (item) this.router.navigate([`estados/edit/${item.id}`])
    else this.router.navigate([`estados/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: EstadoInterface) {
    if (item.id) this.router.navigate([`estados/view/${item.id}`])
    else this.router.navigate([`estados/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: EstadoInterface) {
  ...
  removeItem(item: EstadoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/estados/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: EstadoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/estados', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/estados?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
