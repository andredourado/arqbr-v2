...
export class FrequenciaListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'espacoEmDias', label: 'Em dias', type: 'number' }
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getFrequencia())
  }
  ...
  getFrequencia() {
    this.httpClient.get(`${environment.baseUrl}/frequencias?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/frequencias?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: FrequenciaInterface) {
    if (item) this.router.navigate([`frequencias/edit/${item.id}`])
    else this.router.navigate([`frequencias/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: FrequenciaInterface) {
    if (item.id) this.router.navigate([`frequencias/view/${item.id}`])
    else this.router.navigate([`frequencias/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: FrequenciaInterface) {
  ...
  removeItem(item: FrequenciaInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/frequencias/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: FrequenciaInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/frequencias', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/frequencias?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
