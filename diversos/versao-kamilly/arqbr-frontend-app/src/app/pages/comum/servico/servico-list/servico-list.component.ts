...
export class ServicoListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getServico())
  }
  ...
  getServico() {
    this.httpClient.get(`${environment.baseUrl}/servicos?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/servicos?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: ServicoInterface) {
    if (item) this.router.navigate([`servicos/edit/${item.id}`])
    else this.router.navigate([`servicos/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: ServicoInterface) {
    if (item.id) this.router.navigate([`servicos/view/${item.id}`])
    else this.router.navigate([`servicos/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: ServicoInterface) {
  ...
  removeItem(item: ServicoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/servicos/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: ServicoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/servicos', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/servicos?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
