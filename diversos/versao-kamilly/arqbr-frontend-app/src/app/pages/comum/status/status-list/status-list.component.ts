...
export class StatusListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'sequencia', label: 'Sequência' },
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getStatus())
  }
  ...
  getStatus() {
    this.httpClient.get(`${environment.baseUrl}/statuses?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/statuses?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: StatusInterface) {
    if (item) this.router.navigate([`statuses/edit/${item.id}`])
    else this.router.navigate([`statuses/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: StatusInterface) {
    if (item.id) this.router.navigate([`statuses/view/${item.id}`])
    else this.router.navigate([`statuses/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: StatusInterface) {
  ...
  removeItem(item: StatusInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/statuses/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: StatusInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/statuses', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/statuses?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
