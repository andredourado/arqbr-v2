...
@Component({
  selector: "/tipo-afastamento-list",
  templateUrl: ".//tipo-afastamento-list.component.html",
})
export class TipoAfastamentoListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getTipoAfastamento())
  }
  ...
  getTipoAfastamento() {
    this.httpClient.get(`${environment.baseUrl}/tipos-afastamento?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/tipos-afastamento?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: TipoAfastamentoInterface) {
    if (item) this.router.navigate([`tipos-afastamento/edit/${item.id}`])
    else this.router.navigate([`tipos-afastamento/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: TipoAfastamentoInterface) {
    if (item.id) this.router.navigate([`tipos-afastamento/view/${item.id}`])
    else this.router.navigate([`tipos-afastamento/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: TipoAfastamentoInterface) {
  ...
  removeItem(item: TipoAfastamentoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/tipos-afastamento/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: TipoAfastamentoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/tipos-afastamento', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/tipos-afastamento?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
