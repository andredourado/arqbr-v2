...
@Component({
  selector: "/unidade-sla-list",
  templateUrl: ".//unidade-sla-list.component.html",
})
export class UnidadeSlaListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getUnidadeSla())
  }
  ...
  getUnidadeSla() {
    this.httpClient.get(`${environment.baseUrl}/unidades-sla?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/unidades-sla?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: UnidadeSlaInterface) {
    if (item) this.router.navigate([`unidades-sla/edit/${item.id}`])
    else this.router.navigate([`unidades-sla/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: UnidadeSlaInterface) {
    if (item.id) this.router.navigate([`unidades-sla/view/${item.id}`])
    else this.router.navigate([`unidades-sla/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: UnidadeSlaInterface) {
  ...
  removeItem(item: UnidadeSlaInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/unidades-sla/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: UnidadeSlaInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/unidades-sla', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/unidades-sla?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
