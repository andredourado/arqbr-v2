...
export class DocumentoDigitalCampoListComponent implements OnInit, OnDestroy {
  @ViewChild(PoTableComponent, { static: true }) table: PoTableComponent
  public readonly fields: Array<any> = [
    { property: "id", key: true, visible: false },
    { property: 'documentoDigitalNip', label: 'Documento Digital' },
    { property: 'campoDocumentoNomeCampo', label: 'Campo' },
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getDocumentoDigitalCampo())
  }
  ...
  getDocumentoDigitalCampo() {
    this.httpClient.get(`${environment.baseUrl}/documentos-digitais-campos?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/documentos-digitais-campos?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: DocumentoDigitalCampoInterface) {
    if (item) this.router.navigate([`documentos-digitais-campos/edit/${item.id}`])
    else this.router.navigate([`documentos-digitais-campos/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: DocumentoDigitalCampoInterface) {
    if (item.id) this.router.navigate([`documentos-digitais-campos/view/${item.id}`])
    else this.router.navigate([`documentos-digitais-campos/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: DocumentoDigitalCampoInterface) {
  ...
  removeItem(item: DocumentoDigitalCampoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/documentos-digitais-campos/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: DocumentoDigitalCampoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/documentos-digitais-campos', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/documentos-digitais-campos?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
