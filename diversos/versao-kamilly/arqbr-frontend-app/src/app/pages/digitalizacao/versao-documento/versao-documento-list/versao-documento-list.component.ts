...
@Component({
  selector: "/versao-documento-list",
  templateUrl: ".//versao-documento-list.component.html",
})
export class VersaoDocumentoListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'tipoDocumentoDescricao', label: 'Tipo de Documento' },
    { property: 'descricaoVersao', label: 'Descrição da Versão' },
    { property: 'qrcode', label: 'Qr Code' },
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getVersaoDocumento())
  }
  ...
  getVersaoDocumento() {
    this.httpClient.get(`${environment.baseUrl}/versoes-documento?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/versoes-documento?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: VersaoDocumentoInterface) {
    if (item) this.router.navigate([`versoes-documento/edit/${item.id}`])
    else this.router.navigate([`versoes-documento/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: VersaoDocumentoInterface) {
    if (item.id) this.router.navigate([`versoes-documento/view/${item.id}`])
    else this.router.navigate([`versoes-documento/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: VersaoDocumentoInterface) {
  ...
  removeItem(item: VersaoDocumentoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/versoes-documento/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: VersaoDocumentoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/versoes-documento', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/versoes-documento?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
