...
export class AfastamentoListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'tipoAfastamentoDescricao', label: 'Tipo' },
    { property: 'inicio', label: 'Início', type: 'date' },
    { property: 'fim', label: 'Fim', type: 'date' }
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getAfastamento())
  }
  ...
  getAfastamento() {
    this.httpClient.get(`${environment.baseUrl}/afastamentos?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/afastamentos?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: AfastamentoInterface) {
    if (item) this.router.navigate([`afastamentos/edit/${item.id}`])
    else this.router.navigate([`afastamentos/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: AfastamentoInterface) {
    if (item.id) this.router.navigate([`afastamentos/view/${item.id}`])
    else this.router.navigate([`afastamentos/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: AfastamentoInterface) {
  ...
  removeItem(item: AfastamentoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/afastamentos/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: AfastamentoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/afastamentos', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/afastamentos?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
