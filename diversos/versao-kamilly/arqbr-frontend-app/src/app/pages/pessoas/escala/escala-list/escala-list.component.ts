...
export class EscalaListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'jornadaDescricao', label: 'Jornada' },
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getEscala())
  }
  ...
  getEscala() {
    this.httpClient.get(`${environment.baseUrl}/escalas?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/escalas?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: EscalaInterface) {
    if (item) this.router.navigate([`escalas/edit/${item.id}`])
    else this.router.navigate([`escalas/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: EscalaInterface) {
    if (item.id) this.router.navigate([`escalas/view/${item.id}`])
    else this.router.navigate([`escalas/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: EscalaInterface) {
  ...
  removeItem(item: EscalaInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/escalas/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: EscalaInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/escalas', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/escalas?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
