...
export class FuncaoListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getFuncao())
  }
  ...
  getFuncao() {
    this.httpClient.get(`${environment.baseUrl}/funcoes?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/funcoes?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: FuncaoInterface) {
    if (item) this.router.navigate([`funcoes/edit/${item.id}`])
    else this.router.navigate([`funcoes/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: FuncaoInterface) {
    if (item.id) this.router.navigate([`funcoes/view/${item.id}`])
    else this.router.navigate([`funcoes/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: FuncaoInterface) {
  ...
  removeItem(item: FuncaoInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/funcoes/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: FuncaoInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/funcoes', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/funcoes?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
