...
@Component({
  selector: "/jornada-list",
  templateUrl: ".//jornada-list.component.html",
})
export class JornadaListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getJornada())
  }
  ...
  getJornada() {
    this.httpClient.get(`${environment.baseUrl}/jornadas?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/jornadas?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: JornadaInterface) {
    if (item) this.router.navigate([`jornadas/edit/${item.id}`])
    else this.router.navigate([`jornadas/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: JornadaInterface) {
    if (item.id) this.router.navigate([`jornadas/view/${item.id}`])
    else this.router.navigate([`jornadas/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: JornadaInterface) {
  ...
  removeItem(item: JornadaInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/jornadas/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: JornadaInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/jornadas', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/jornadas?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
