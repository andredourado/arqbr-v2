...
export class PessoaListComponent implements OnInit, OnDestroy {
  ...
  public readonly fields: Array<any> = [
    ...
    { property: 'funcaoDescricao', label: 'Função' },
  ]
  ...
  ngOnInit() {
    ...
    this.subscriptions.add(this.getPessoa())
  }
  ...
  getPessoa() {
    this.httpClient.get(`${environment.baseUrl}/pessoas?page=1&pageSize=10`)
    .pipe(finalize(() => {
      this.loading = false
  ...
  search(search: string) {
    ...
    this.subscriptions.add(
      this.httpClient.get(`${environment.baseUrl}/pessoas?page=1&pageSize=10&search=${search}`)
        .subscribe({
  ...
  editItem(item: PessoaInterface) {
    if (item) this.router.navigate([`pessoas/edit/${item.id}`])
    else this.router.navigate([`pessoas/edit/${this.getSelectedItemsKeys()[0].id}`])
  }

  viewItem(item: PessoaInterface) {
    if (item.id) this.router.navigate([`pessoas/view/${item.id}`])
    else this.router.navigate([`pessoas/view/${this.getSelectedItemsKeys()[0].id}`])
  }

  excludeItem(item: PessoaInterface) {
  ...
  removeItem(item: PessoaInterface) {
    this.subscriptions.add(
      this.httpClient.delete(`${environment.baseUrl}/pessoas/${item.id}`)
        .subscribe({
          next: (response: ListResponse) => {
  ...
  removeItems(ids: PessoaInterface[]) {
    this.subscriptions.add(
      this.restService.deleteAll('/pessoas', ids)
        .subscribe({
          next: (response: ListResponse) => {
            this.poNotificationService.success({
  ...
  showMoreItems() {
    if (this.hasNext) {
      this.loading = true
      this.subscriptions.add(
        this.httpClient.get(`${environment.baseUrl}/pessoas?page=${this.page += 1}&pageSize=10&search=${this.filter}`)
